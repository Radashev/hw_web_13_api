...
# def create_contact(contact: schemas.ContactCreate, db: Session = Depends(get_db), Authorize: AuthJWT = Depends()):
...
#                   email: Optional[str] = None, db: Session = Depends(get_db), Authorize: AuthJWT = Depends()):
#     Authorize.jwt_required()
#     current_user = Authorize.get_jwt_subject()
#     db_user = crud.get_user_by_email(db, email=current_user)
...
# def read_contact(contact_id: int, db: Session = Depends(get_db), Authorize: AuthJWT = Depends()):
#     Authorize.jwt_required()
#     current_user = Authorize.get_jwt_subject()
#     db_user = crud.get_user_by_email(db, email=current_user)
...
# def delete_contact(contact_id: int, db: Session = Depends(get_db), Authorize: AuthJWT = Depends()):
#     Authorize.jwt_required()
#     current_user = Authorize.get_jwt_subject()
#     db_user = crud.get_user_by_email(db, email=current_user)
...
# def read_contacts_with_upcoming_birthdays(days: int = 7, db: Session = Depends(get_db), Authorize: AuthJWT = Depends()):
#     Authorize.jwt_required()
#     current_user = Authorize.get_jwt_subject()
#     db_user = crud.get_user_by_email(db, email=current_user)
...
# import os  # Додано
# import patch_pydantic  # Патч для сумісності з Python 3.12
#
# from fastapi import FastAPI, Depends, HTTPException, status
#
# from app import models, schemas, crud
...
# DATABASE_URL = os.getenv('DATABASE_URL')  # Додано
# SECRET_KEY = os.getenv('SECRET_KEY')  # Додано
#
# # Створення синхронного двигуна для створення таблиць
# sync_engine = create_engine(DATABASE_URL.replace('asyncpg', 'postgresql'))
#
# # Створення бази даних
# models.Base.metadata.create_all(bind=engine)
#
...
# # Налаштування для JWT
# class Settings(BaseModel):
#     authjwt_secret_key: str = SECRET_KEY  # Використання змінної оточення
#     authjwt_access_token_expires: int = 15  # in minutes
#     authjwt_refresh_token_expires: int = 30  # in days
#
#
